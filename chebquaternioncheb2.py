...
class QuaternionTransformer(nn.Module):
    ...
    def generate(self, prompt, max_new_tokens=50, seq_len=32, return_text=True):
        self.eval()
        ...
        for _ in range(max_new_tokens):
            ...
            #generated_2.append(next_token2)
        ...
        if return_text:
            ...
            return self.tokenizer_wrapper.tokenizer.decode(generated, skip_special_tokens=True)
    ...
    def forward(self, src, tgt=None, mode="eval"):
        ...
        for layer in self.encoder_layers:
            src_embed = cp.checkpoint(layer, src_embed, src_pad_mask)
        ...
        for layer in self.decoder_layers:
            tgt_embed = cp.checkpoint(layer, tgt_embed, src_embed, tgt_pad_mask, src_pad_mask)
...
class CatalanLossCriterion(nn.Module):
    ...
    def forward(self, output, target):
        ...
        one_hot = F.one_hot(target.long(), num_classes=output.size(-1)).float().to(output.device)
...
def train_model(batch_size, model, dataset, optimizer_std, optimizer_q, catalan_optimizer, catalan_criterion, criterion, device):
    model.train()
    total_loss = 0
    n = 0
...
def train_model_catalan(batch_size, model, dataset, optimizer_std, optimizer_q, catalan_optimizer, catalan_criterion, use_quaternion, device):
    model.train()
    total_loss = 0
    n = 0
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        try:
            src = src.to(device)
            target = target.to(device)
            #min_len = min(src.size(1), target.size(1))
            src = src[:, :seq_len]

            target = target[:, :seq_len]  # Truncate targets if too long
            #print(f"src shape: {src.shape}")
            #print(f"target shape: {target.shape}")
            target_labels = target.to(device)
            ...
            #loss = criterion(output.reshape(-1, output.shape[-1]), target_labels.reshape(-1, target_labels.shape[-1]))
            logits_filtered = output.reshape(-1, output.shape[2])                    # [B*T, V]
            #print(f"logits_filtered reshaped: {logits_filtered.shape}")
            targets_filtered = target_labels[:, :seq_len]  # Truncate targets if too long
            #print(f"targets_filtered rehaped: {targets_filtered.shape}")
            targets_filtered = targets_filtered.reshape(-1)                         
            #print(f"targets_filtered reshaped: {targets_filtered.shape}")
            ...
            if use_quaternion == True:
                    ...
                    #loss = F.smooth_l1_loss(logits_filtered, predicted_targets)
                    #loss = quaternion_cross_entropy(logits_filtered, predicted_targets)  # Use quaternion loss function
                    #loss += quaternion_mse_loss(logits_filtered, predicted_targets)  # Use quaternion loss function    
                    ce_loss = F.cross_entropy(logits_filtered, predicted_targets)
                    #reg_loss = catalan_criterion.forward_regressive(logits_filtered, predicted_targets)
                    reg_loss = catalan_criterion.forward_regressive(output.softmax(-1), F.one_hot(target.long(), num_classes=output.size(-1)).float())
                    #loss = quaternion_angular_loss(logits_filtered, predicted_targets)  # Use quaternion loss function
            else:
                    ce_loss = F.cross_entropy(logits_filtered, targets_filtered)
                    reg_loss = catalan_criterion.forward_regressive(output.softmax(-1), F.one_hot(target.long(), num_classes=output.size(-1)).float())
                    #reg_loss = catalan_criterion.forward_regressive(logits_filtered, predicted_targets) 
                    #reg_loss = catalan_criterion.forward_regressive(logits_filtered.softmax(-1), F.one_hot(logits.long(), num_classes=logits.size(-1)).float())
            ...
            if n % 100 ==0:
                    ...
                    if use_quaternion == False:
                        ...
                        generated_text = generate_2(model,prompt, base_tokenizer, seq_len, device)
                    else:
                        generated_text = model.generate(prompt, max_new_tokens=100, seq_len=32)
...
def train_decoder_autoregressive(model, dataset, tokenizer, optimizer_std, optimizer_q, catalan_optimizer, catalan_criterion, use_quaternion, loss_fn, batch_size, seq_len, device):
    model.train()
    total_loss = 0
    ...
    for i in range(0, len(dataset), batch_size):
        ...
        for t in range(2, max_len):
            ...
            try:
                if use_quaternion == True:
                    logits, predicted_targets= forward_fn(tgt_ids)
                else:
                    logits = model(src)
                #print(f"Logits shape: {logits.shape}")
                ...
                if use_quaternion == True:
                    ...
                    #ce_loss = quaternion_cross_entropy(logits_filtered, predicted_targets)  # Use quaternion loss function
                    #ce_loss += quaternion_mse_loss(logits_filtered, predicted_targets)  # Use quaternion loss function    
                    #ce_loss = F.cross_entropy(logits_filtered, predicted_targets)
                    #reg_loss = catalan_criterion.forward_regressive(logits_filtered, predicted_targets)
                    ce_loss = quaternion_angular_loss(logits_filtered, predicted_targets)  # Use quaternion loss function
                    reg_loss = catalan_criterion.forward_regressive(logits.softmax(-1), F.one_hot(tgt_ids.long(), num_classes=logits.size(-1)).float())
                ...
                step_loss = 0.5 * ce_loss + 0.5 * reg_loss
...
def generate_2(model, prompt, tokenizer, seq_len, device, max_generated=120, repetition_penalty=1.2, top_p=0.9):
    ...
    with torch.no_grad():
        ...
        for _ in range(max_generated):
            ...
            outputs = model(encoder_input_ids)
...
def main():
    ...
    parser.add_argument('--data', type=str, default=r"C:\Users\abias\alldatareasoning\iimath\1\infinityinstructmath1.json", help='Path to JSON data')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.use_quaternion == False:
            avg_loss = train_decoder_autoregressive(
                model, dataset, tokenizer, optimizer_std, optimizer_q, catalan_optimizer, catalan_criterion, args.use_quaternion, criterion,
                args.batch_size, args.max_seq_length, device
            )
        if args.use_quaternion == True:
            avg_loss = train_model_catalan(args.batch_size, model, dataset, optimizer_std, optimizer_q, catalan_optimizer, catalan_criterion, args.use_quaternion, device)
